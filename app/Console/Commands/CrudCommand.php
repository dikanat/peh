...
class CrudCommand extends Command
{
    ...
    public function handle() {
        ...
        $this->call('crud:storerequest', ['name' => $modelName . '\StoreRequest', '--crud-name' => $name, '--model-name' => $modelName, '--model-namespace' => $modelNamespace, '--view-path' => $viewPath, '--route-group' => $routeGroup, '--pagination' => $perPage, '--fields' => $fields, '--validations' => $validations]);
        $this->call('crud:controller', ['name' => $modelName . 'Controller', '--crud-name' => $name, '--model-name' => $modelName, '--model-namespace' => $modelNamespace, '--view-path' => $viewPath, '--route-group' => $routeGroup, '--pagination' => $perPage, '--fields' => $fields, '--validations' => $validations]);
    ...
    protected function addRoutes() {
      ...
      $modelName = Str::singular($name);
        return ["
        Route::group([
          'as' => 'system.main." . $this->routeName . ".',
          'prefix' => 'dashboard/" . $this->routeName . "',
          'namespace' => 'Backend\Main',
        ], function(){
          Route::get('status-done/{id}', '" . $modelName . "Controller@status_done')->name('status-done');
          Route::get('status-pending/{id}', '" . $modelName . "Controller@status_pending')->name('status-pending');
          Route::get('enable/{id}', '" . $modelName . "Controller@enable')->name('enable');
          Route::get('disable/{id}', '" . $modelName . "Controller@disable')->name('disable');
          Route::get('status/{id}/{slug}', '" . $modelName . "Controller@status')->name('status');
          Route::get('delete/{id}', '" . $modelName . "Controller@delete')->name('delete');
          Route::get('deleteall', '" . $modelName . "Controller@deleteall')->name('deleteall');
          Route::resource('/', '" . $modelName . "Controller')->parameters(['' => 'id']);
        });
        "];
    }
